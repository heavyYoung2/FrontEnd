import React, { useCallback } from 'react';
import CouncilQrScanner, {
  CouncilQrScannerResult,
} from '@/src/components/CouncilQrScanner';
import {
  rentItemByQrToken,
  RENTAL_SCAN_ERROR_MESSAGES,
  RentalQrScanError,
} from '@/src/api/rentalAdmin';

type RentalQrScannerProps = {
  title?: string;
  showBack?: boolean;
  backFallbackHref?: string;
  showHeaderRefresh?: boolean;
  instruction?: string;
};

const DEFAULT_TITLE = 'QR코드 스캔';
const DEFAULT_INSTRUCTION = '학생 QR을 스캔하면 대여가 즉시 처리됩니다.';

export default function RentalQrScanner({
  title = DEFAULT_TITLE,
  showBack = false,
  backFallbackHref,
  showHeaderRefresh = false,
  instruction = DEFAULT_INSTRUCTION,
}: RentalQrScannerProps) {
  const handleProcess = useCallback(async (token: string): Promise<CouncilQrScannerResult> => {
    try {
      await rentItemByQrToken(token);
      return {
        status: 'success',
        message: '대여 완료',
        actionLabel: '대여 완료',
        speech: '대여가 완료되었습니다',
      };
    } catch (err) {
      if (err instanceof RentalQrScanError) {
        const friendly =
          (err.code && RENTAL_SCAN_ERROR_MESSAGES[err.code]) ||
          (err.message && err.message.trim().length > 0 ? err.message : null);
        if (err.code && RENTAL_SCAN_ERROR_MESSAGES[err.code]) {
          return {
            status: 'denied',
            message: friendly ?? '대여가 불가합니다.',
            actionLabel: '확인',
            speech: '대여가 불가합니다',
          };
        }
        return {
          status: 'invalid',
          message: friendly ?? '대여 정보를 확인할 수 없어요.',
          actionLabel: '다시 시도',
        };
      }
      const message =
        err instanceof Error && err.message ? err.message : '대여 정보를 확인할 수 없어요.';
      return {
        status: 'invalid',
        message,
        actionLabel: '다시 시도',
      };
    }
  }, []);

  return (
    <CouncilQrScanner
      headerProps={{
        title,
        showBack,
        backFallbackHref,
      }}
      instruction={instruction}
      processingText="인식 중..."
      successChipText="대여 완료"
      inlineResetLabel="다시 스캔"
      successActionLabel="대여 완료"
      deniedActionLabel="확인"
      invalidActionLabel="다시 시도"
      successSpeech="대여가 완료되었습니다"
      deniedSpeech="대여가 불가합니다"
      showHeaderRefresh={showHeaderRefresh}
      onProcess={handleProcess}
    />
  );
}
