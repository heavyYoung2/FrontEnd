...
export default function StudentHome() {
  ...
  const fetchList = useCallback(async () => {
    try {
      ...
      console.warn('[student getAdminEvents] fail', e);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const onRefresh = useCallback(async () => {
    try {
      ...
      console.warn('[student refresh getAdminEvents] fail', e);
    } finally {
      setRefreshing(false);
    }
  }, []);
  ...
  const renderItem = ({ item }: { item: AdminEventInfo }) => (
    <Pressable
      onPress={() =>
        router.push({
          pathname: '/notice/[id]',
          params: { id: String(item.eventId), readonly: '1', role: 'student' },
        })
      }
    ...
    <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
      <CouncilHeader
        ...
        right={(
          <Pressable
            onPress={() =>
              router.push({ pathname: '/notice/calendar', params: { role: 'student' } })
            }
            hitSlop={10}
